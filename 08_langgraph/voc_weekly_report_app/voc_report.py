...
def validate_ticket_data(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing_columns:
        raise TicketDataError("필수 열이 없다: " + ", ".join(missing_columns))
    if dataframe.empty:
...
def calculate_support_metrics(dataframe: pd.DataFrame) -> dict:
    ...
    return {
        "period": (
            f"{tickets['received_date'].min()} ~ " f"{tickets['received_date'].max()}"
        ),
        ...
        "avg_resolution_hours": (
            round(float(resolved["resolution_hours"].mean()), 1)
            if resolved_count
            else 0.0
        ),
        "sla_violation_count": sla_violation_count,
        "sla_violation_rate_pct": (
            round(sla_violation_count / resolved_count * 100, 1)
            if resolved_count
            else 0.0
        ),
        "avg_satisfaction": (
            round(float(satisfaction.mean()), 1) if not satisfaction.empty else 0.0
        ),
...
def run_report_pipeline(dataframe: pd.DataFrame, request: str) -> dict:
    ...
    def analyst_node(state: ReportState) -> dict:
        analyst_result = analyst_agent.invoke(
            {"messages": [{"role": "user", "content": state["request"]}]}  # type: ignore
        )
    ...
    def save_node(state: ReportState) -> dict:
        report_markdown = render_report_markdown(
            state["metrics"], state["report_sections"] # type: ignore
        )
        try:
    ...
    if not all(
        heading in final_state["report_markdown"] for heading in required_headings
    ):
